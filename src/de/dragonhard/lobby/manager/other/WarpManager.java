...
public class WarpManager extends Managers {
    ...
    public void getWarpList(Player p){ //call for a list of all the warps from a player
        ...
        if(playerManager.getCurrentWarps(p) == 0 && playerManager.getMaxWarps(p) == 0){p.sendMessage("§4Du kannst keine Warps setzen!");return;}
        if(playerManager.getCurrentWarps(p) == 0 && playerManager.getMaxWarps(p) != 0){p.sendMessage("§4Du hast keine Warps gesetzt!");return;}
    ...
    public void delWarp(Player p, String warpName){//call to delete a existing Warp
        ...
        remove(warpReader.getFile(warpName));
    ...
    public void delAllWarps(Player p){
        ...
        for(int i = 0; i < this.getPlayerManager().getMaxWarps(p); i++) {
            ...
            if(!warpReader.exists(p,warpListReader.getString("warp_"+i))){
                warpListReader.set("warp_" + i, "frei");
            }else{
